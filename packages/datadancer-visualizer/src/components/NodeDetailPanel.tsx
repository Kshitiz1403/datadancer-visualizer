...
const NodeDetailPanel: React.FC<NodeDetailPanelProps> = ({ isOpen, nodeData, onClose, darkMode = false }) => {
  ...
  const getTriggeredErrorHandler = () => {
    if (!hasError || !definitionState.onErrors || !executionState) return null;
                  ...
                  {definitionState.dataConditions && (
                    <div className="conditions-list">
                      ...
                      {definitionState.defaultCondition && (
                        <div
                          className={`condition-item ${wasExecuted && executionState?.matchedCondition === 'default' ? 'matched' : 'unmatched'}`}
                        >
                          <div className="condition-header">
