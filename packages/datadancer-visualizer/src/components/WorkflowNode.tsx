...
import { WorkflowThemeContext } from './WorkflowGraph';
import {
  Clock,
  AlertCircle,
  Activity,
...
const WorkflowNode: React.FC<WorkflowNodeProps> = ({ data, onNodeClick, isSelected, id }) => {
  const theme = useContext(WorkflowThemeContext);
  ...
  const nodeColor = getNodeTypeColor(state.type, hasError, wasExecuted, theme);

  const getTypeIcon = () => {
    const iconProps = { size: 16, style: { color: nodeColor } };
    switch (state.type) {
      case 'operation': return <Activity {...iconProps} />;
